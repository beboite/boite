...
| File | What it holds |
|---|---|
| `packages/ui/src/lib/strings.ts` | English. Every user-facing sentence of the UI, and the shape every translation mirrors. |
...
Nothing else imports `./strings` directly. `lib/i18n.svelte` is the one door,
and it also re-exports `fill`, which fills the `{name}` slots of a sentence.

## Adding a sentence

Put it in `strings.ts`, under the block its screen belongs to, then in
