...
Boite opens a tour the first time it runs on a device: six screens, each one
...
A seventh screen, dictation, is written and translated and does not show:
...
2. A block under `onboarding` in `lib/strings.ts`, with a `title`, then the
