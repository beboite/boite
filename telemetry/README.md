...
`boite-telemetry`. Until a custom domain is chosen it is served on a
workers.dev subdomain; do not invent one in `wrangler.toml`.
...
This Worker is Boite's. It does not share a PostHog project, a hostname, or
rate-limit namespaces with any other app.
...
The webview never talks to this Worker. The Tauri window and `boite-server`
do. A phone uses the same bus over WebSocket; the host process is the one that
POSTs.
...
- Every property is validated in `buildBatch` before it is forwarded, and any
  ...
  `[a-z0-9_]{1,40}`, versions `[A-Za-z0-9.+-]{1,32}`; counts must be finite and
  non-negative. The app maps these onto closed vocabularies before sending, and
  this is the half of that guarantee that does not assume the client is the one
  we shipped.
...
- `wrangler` CLI installed and logged in (`bun install` in this directory, then
  `npx wrangler login`)
...
- `wrangler.toml`: keep `workers_dev = true` until you have a hostname of your
  own, then add a `routes` block. Do not commit a guessed domain.
...
- `wrangler.toml`: replace `ALERT_FROM` with a Resend-verified sender to
  exit sandbox mode.
- Rate-limit `namespace_id` values must be unique in the Cloudflare account.
  The 2001/2003/2006/2005 range is this Worker's; do not reuse another app's.
...
The compiled desktop binary reads `BOITE_TELEMETRY_URL` at build time. A GitHub
...
- A `/track` batch is rejected with 502 when PostHog is unreachable. The client
  keeps the batch in RAM and retries with backoff; it never writes events to
  disk. This is deliberate: nothing about telemetry is worth persisting to a
  user's disk.

## Tests

The `buildBatch` tests live in `src/index.test.ts` and run with the rest of the
app's Vitest suite. They need no network, no secret, and no Wrangler.

## Licence

Same licence as the parent Boite repository ([MIT](../LICENSE)).
