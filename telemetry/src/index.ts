...
const ALLOWED_EVENTS = new Set(["ping", "first_run", "app_launched", "session_ended", "thread_spawned", "project_added", "turn_finished"]);
...
export default {
  async fetch(request: Request, env: Env, ctx: ExecutionContext): Promise<Response> {
    ...
    try {
      ...
      switch (url.pathname) {
        case "/":
          response = json({ ok: true, service: "boite-telemetry" });
          break;
        case "/track":
          response =
...
async function handleTrack(request: Request, env: Env, ctx: ExecutionContext): Promise<Response> {
  ...
  const usable = payload.events.filter(
    (e) => typeof e?.name === "string" && ALLOWED_EVENTS.has(e.name),
  );
  if (usable.length === 0) {
    return json({ error: "bad_payload" }, 400);
  }
  ...
  if (!sent.ok) {
    console.error("posthog capture failed", sent.status);
...
export function buildBatch(
  ...
  return events.filter(ev => ALLOWED_EVENTS.has(ev.name)).map((ev) => {
    ...
    const properties: Record<string, unknown> = {
      ...
      os: ["windows", "macos", "linux"].includes(ev.os ?? "") ? ev.os : "other",
      arch: ["x64", "arm64", "ia32"].includes(ev.arch ?? "") ? ev.arch : "other",
    };
    if (ev.name === "thread_spawned" || ev.name === "turn_finished") {
      properties.provider = providers.includes(ev.provider ?? "") ? ev.provider : "other";
    }
    ...
    if (["app_launched", "session_ended", "turn_finished"].includes(ev.name)) {
      ...
      if (duration !== undefined) properties.duration_ms = Math.min(604800000, Math.round(duration));
    }
    return {
      event: ev.name, distinct_id: distinctId,
      ...(isUuidV4(ev.uuid) ? { uuid: ev.uuid } : {}),
      timestamp: eventTimestamp(ev.client_ts, timestamp), properties,
    };
  });
}
...
function posthogApiHeaders(env: Env): Record<string, string> {
  return {
...
async function posthogQuery(env: Env, query: string, name: string): Promise<QueryResult> {
  ...
  try {
    const res = await fetch(url, {
      method: "POST",
      headers: posthogApiHeaders(env),
      body: JSON.stringify({ query: { kind: "HogQLQuery", query }, name, refresh: "force_blocking" }),
      signal: AbortSignal.timeout(POSTHOG_TIMEOUT_MS),
    });
    if (!res.ok) {
...
function checkUa(request: Request, env: Env): Response | null {
  const prefix = env.UA_PREFIX || "Boite/";
  const ua = request.headers.get("User-Agent") ?? "";
...
async function notifyRateLimit(env: Env, endpoint: string, ip: string): Promise<void> {
  ...
  const subject = `[Boite telemetry] rate limit hit on ${endpoint}`;
  const text = [
    `Endpoint: ${endpoint}`,
    `Masked IP: ${maskIp(ip)}`,
