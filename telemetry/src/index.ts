...
export interface TelemetryEvent {
  ...
  thread_count?: number;
  live_pty_count?: number;
  ui_language?: string;
  theme?: string;
  thread_worktrees?: boolean;
  animations?: string;
  mcp_yolo?: boolean;
  idle_autoclose?: boolean;
...
export function buildBatch(
  ...
  return events.map((ev) => {
    ...
    const optional: Array<[string, unknown]> = [
      ...
      ["kind", code(ev.kind)],
      ["provider", code(ev.provider)],
      ["pane_kind", code(ev.pane_kind)],
      ["project_count", count(ev.project_count)],
      ["thread_count", count(ev.thread_count)],
      ["live_pty_count", count(ev.live_pty_count)],
      ["ui_language", code(ev.ui_language)],
      ["theme", code(ev.theme)],
      ["thread_worktrees", flag(ev.thread_worktrees)],
      ["animations", code(ev.animations)],
      ["mcp_yolo", flag(ev.mcp_yolo)],
      ["idle_autoclose", flag(ev.idle_autoclose)],
      ["orchestrator", flag(ev.orchestrator)],
      ["voice", flag(ev.voice)],
    ];
