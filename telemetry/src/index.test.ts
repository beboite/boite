...
describe("buildBatch", () => {
  ...
  it("omits optional fields instead of sending them empty", () => {
    ...
    expect(item!.properties).not.toHaveProperty("kind");
  });

  it("forwards the event properties the app reports", () => {
    const spawned: TelemetryEvent = {
      name: "thread_spawned",
      app_version: "1.2.3",
      os_version: "Windows 11",
      os: "windows",
    ...
    const [item] = buildBatch("A", [spawned], IDS, "FR", TS);

    expect(item!.properties.os).toBe("windows");
    expect(item!.properties.arch).toBe("x86_64");
    expect(item!.properties.surface).toBe("desktop");
    expect(item!.properties.kind).toBe("agent");
    expect(item!.properties.provider).toBe("claude");
  });
  ...
  it("drops a property whose shape is not the one the app sends", () => {
    ...
    const hostile = {
      ...
      error_code: "C:\\Users\\alice\\project missing",
      pane_kind: "editor; DROP TABLE",
      duration_ms: -5,
      target_version: "<script>alert(1)</script>",
    } as unknown as TelemetryEvent;
    const [item] = buildBatch("A", [hostile], IDS, "FR", TS);

    expect(item!.properties.operation).toBe("thread_create");
    expect(item!.properties).not.toHaveProperty("error_code");
    expect(item!.properties).not.toHaveProperty("pane_kind");
    expect(item!.properties).not.toHaveProperty("duration_ms");
  ...
  it("keeps a settings snapshot as booleans and codes", () => {
    ...
    const [item] = buildBatch("B", [snapshot], IDS, "FR", TS);

    expect(item!.properties.ui_language).toBe("fr");
    expect(item!.properties.theme).toBe("acrylic_black");
    expect(item!.properties.mcp_yolo).toBe(false);
    // False must survive: `if (ev.mcp_yolo)` would have dropped it.
    expect(item!.properties).toHaveProperty("mcp_yolo");
    expect(item!.properties.voice).toBe(false);
  });
