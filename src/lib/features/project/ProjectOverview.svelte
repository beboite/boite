...
<div class="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
  ...
  <section class="rounded-lg border border-border bg-[var(--color-surface)] p-3">
    ...
    {#if threads.length === 0}
      <p class="text-sm text-muted-foreground">{t("project.noThreads")}</p>
    {:else}
      <ul class="flex max-h-56 flex-col gap-0.5 overflow-y-auto">
        {#each threads as thread (thread.id)}
          <li>
            <button
              type="button"
              class="flex w-full items-center gap-2 rounded px-1.5 py-1 text-left text-base text-foreground/85 transition hover:bg-accent hover:text-foreground"
  ...
  <section class="rounded-lg border border-border bg-[var(--color-surface)] p-3">
    ...
    {:else}
      <p class="truncate text-base font-medium text-foreground" title={git.branch ?? ""}>
      ...
      {#if git.log.length > 0}
        <ul class="mt-2 flex flex-col gap-0.5">
          {#each git.log.slice(0, 4) as commit (commit.sha)}
            <li class="truncate text-sm text-muted-foreground" title={commit.summary}>
  ...
  <section class="rounded-lg border border-border bg-[var(--color-surface)] p-3">
    ...
    {:else}
      {#if claimedTodos.length > 0}
        <p class="mb-1.5 text-sm text-[var(--color-warning)]">
  ...
  <p
    class="truncate px-1 font-mono text-xs text-muted-foreground/70 sm:col-span-2 lg:col-span-3"
    title={project.cwd}
  >
