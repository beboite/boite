 ...
 * it. The MCP endpoint is the one thing that does not: `todo_add`, `thread_move`,
 * `worktree_branch`, `project_create` and `thread_spawn` change the app from the
 * outside, and until now the only trace was the result appearing somewhere with
 * nothing to say who did it or when.
 *
...
/** Surfaces an agent's call can be about. Kept as a closed set so a component
    can ask for its own without inventing a key nothing writes. */
 ...
 * scoped per thread through `BOITE_THREAD_ID`, but a call can arrive from a
 * process that was launched outside Boite, and an unattributed call still has to
