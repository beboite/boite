...
/**
 * Installing a command line tool without leaving the settings panel.
 *
 ...
 *
 * One instance per tool: each holds a PTY and a thread id, and two cards sharing
 * one would draw the same log twice. Which of the three actions is a compiler is
 * the caller's business — fastpick's `update` hands the job to fastpick itself,
 * which fetches a signed release, while every plugin action is cargo.
 */
...
/**
 * What a card reads. Every installer here is the same class, but the panels are
 * typed against this rather than against it, so a card cannot reach past what it
 * is meant to draw.
 */
...
/** What one installer needs to know about the tool it installs. */
export type InstallerSpec = {
  /**
   * Names the run: the PTY thread id it holds and the label the row carries.
   *
   * One id per tool for every launch, rather than a fresh one each time.
   ...
   * of starting a second `cargo install` beside the first. Per tool rather than
   ...
   */
  name: string;
  /** Which log this one's warnings land in. */
  scope: string;
  commands: CommandSet;
  /** Told once a run has settled, so whoever probes for the binary can ask again. */
  settled: () => void;
};
...
export class PtyInstaller {
  ...
  /** The PTY thread id, which is also what a remote spawn writes a row under. */
  readonly #threadId: string;

  constructor(spec: InstallerSpec) {
    this.#name = spec.name;
    this.#scope = spec.scope;
    this.#commands = spec.commands;
    this.#settled = spec.settled;
    this.#threadId = `${spec.name}-install`;
  }

  /** Which of the three is running, or the one that last ran. */
  ...
  async cancel(): Promise<void> {
    ...
    const closing = ptyKill(key).catch((err: unknown) => {
      logger.warn(this.#scope, "the install would not be killed", { error: String(err) });
    });
      ...
      const key = await ptyOpen(
        {
          ...
          meta: { projectId: project.id, label: `${this.#name} ${action}`, iconKey: null },
        },
  ...
  #reply(): void {
    ...
    void ptyWrite(this.#key, this.#encoder.encode(data)).catch((err: unknown) => {
      logger.warn(this.#scope, "the build asked something we could not answer", {
        error: String(err),
      });
    });
  }
  ...
  #finish(code: number | null): void {
    ...
    if (code !== 0) {
      logger.warn(this.#scope, `${this.action ?? "install"} exited ${code}`, {
  ...
  #fail(message: string): void {
    ...
    logger.warn(this.#scope, `${this.action ?? "install"} never ran`, { message });
    void this.#forgetRow();
  }
  ...
  async #forgetRow(): Promise<void> {
    ...
    try {
      ...
      logger.warn(this.#scope, "the install thread row stayed behind", { error: String(err) });
    }
  }
}
