...
pub fn run(provider: &Provider, opts: &Options) -> i32 {
    ...
    if !stale.is_empty() {
        if opts.clean {
            ...
            say(
                &format!(
                    "  {} leftover file(s) from an earlier version. Remove with: claude-cc doctor -Provider {} -Clean",
                    stale.len(),
                    provider.id.as_str()
                ),
                Color::Yellow,
    ...
    if plain > 0 && !opts.protect {
        say(
            &format!(
                "  {plain} snapshot(s) in plain text. Fix with: claude-cc doctor -Provider {} -Protect",
                provider.id.as_str()
            ),
            Color::Yellow,
        );
