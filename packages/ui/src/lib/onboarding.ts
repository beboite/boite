...
export type OnboardingStep = 'welcome' | 'agents' | 'voice' | 'usage' | 'reach' | 'quiet' | 'project';
...
const ORDER: readonly OnboardingStep[] = ['welcome', 'agents', 'voice', 'usage', 'reach', 'quiet', 'project'];

/** The screens this build shows, in order. */
export function steps(): OnboardingStep[] {
  return ORDER.filter((step) => step !== 'voice' || VOICE_STEP);
}
