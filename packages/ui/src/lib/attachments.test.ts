...
import { acceptAttachments, decodedBytes, readImageFile, stripDataPrefix } from './attachments';
...
test('a file arrives as base64 with the data: prefix stripped', async () => {
  ...
  const attachment = await readImageFile(new File([bytes], 'pixel.png', { type: 'image/png' }));
...
test('the count cap keeps what fits and names the one left out', () => {
  ...
  expect(refused).toBe(`A turn carries at most ${ATTACHMENTS_PER_TURN} images, so over.png was left out.`);
});
...
test('an image over the size cap is refused by weight, the ones under it still land', () => {
  ...
  expect(refused).toBe('huge.png is too big: an image may weigh 5 MB at most.');
});
...
test('a format no agent reads is refused by name, and an unnamed paste still has a subject', () => {
  ...
  expect(refused).toBe(
    'the pasted image is image/bmp, and an image must be one of image/png, image/jpeg, image/gif, image/webp.'
  );
});
