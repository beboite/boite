import antigravityTile from '../assets/providers/antigravity.png';

/**
 * One mark per provider, the ones T3 Code draws in
 * `apps/web/src/components/Icons.tsx` (MIT, T3 Tools Inc.): the same path data,
 * the same view boxes, the same colours. Each mark belongs to its company. A
 * mark is a brand rather than chrome, so it keeps its own hue where the rest of
 * the UI keeps none. A provider that is not in this map falls back to its
 * initial (`ProviderLogo.svelte`).
 */

/** A fill that follows the theme: `dark` on the dark and grain themes, `light` on light. */
export interface ThemedFill {
  dark: string;
  light: string;
}
...
export type ProviderLogo =
  | {
      kind: 'paths';
      viewBox: string;
      /** A rounded square under the paths, the size of the view box. */
      tile?: { width: number; height: number; rx: number; fill: string };
      paths: LogoPath[];
    }
  | {
      /** A raster mark: T3 Code ships Antigravity's as a 128 px png. */
      kind: 'image';
      viewBox: string;
      width: number;
      height: number;
...
export const providerLogos: Record<string, ProviderLogo> = {
  ...
  // `OpenAI`: Codex is OpenAI's agent.
  codex: {
    kind: 'paths',
    viewBox: '0 0 256 260',
    paths: [
      {
        ...
        fill: { dark: '#ffffff', light: '#000000' }
      }
    ]
  },
  // `OpenCodeIcon`: the hole, then the frame around it.
  opencode: {
    kind: 'paths',
    viewBox: '0 0 32 40',
    paths: [
      { d: 'M24 32H8V16H24V32Z', fill: { dark: '#4B4646', light: '#CFCECD' } },
      { d: 'M24 8H8V32H24V8ZM32 40H0V0H32V40Z', fill: { dark: '#F1ECEC', light: '#211E1E' } }
    ]
  },
  // `GrokIcon`.
  grok: {
  ...
  pi: {
    kind: 'paths',
    viewBox: '0 0 800 800',
    tile: { width: 800, height: 800, rx: 160, fill: '#000' },
    paths: [
      {
      ...
      { d: 'M517.36 400H634.72V634.72H517.36Z', fill: '#fff' }
    ]
  }
};
