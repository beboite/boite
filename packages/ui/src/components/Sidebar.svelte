...
>
  ...
  {#if store.projects.length > 0 && store.owner}
    ...
    >
    {#if projectForm.shown}<div
        class="project-form"
        class:closing={projectForm.closing}
        use:projectForm.attach
        onanimationend={projectForm.end}
      >
...
<style>
  ...
  .project-form.closing {
    animation: leave var(--dur-2) var(--ease-out-quint);
  }
  @keyframes leave {
    to {
      opacity: 0;
