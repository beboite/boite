<script lang="ts">
  ...
  async function pickFavorite(entry: FavoriteModel) {
    ...
    try {
      ...
      if (protocol === 'claude-sdk' || protocol === 'acp' || protocol === 'codex-appserver' || protocol === 'pi' || protocol === 'agy') await store.probeModels(entry.providerId, entry.accountId);
  ...
  $effect(() => {
    ...
    if (shown.protocol !== 'claude-sdk' && shown.protocol !== 'acp' && shown.protocol !== 'codex-appserver' && shown.protocol !== 'pi' && shown.protocol !== 'agy') return;
