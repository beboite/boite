    ...
    {#each ['goal', 'loop'] as kind (kind)}
      ...
      {#if entry}
        <div class="activity-row" data-testid="activity-{kind}">
          ...
          <button type="button" class="ghost icon small" disabled={saving || store.connection !== 'ready'}
            ...
            {#if entry.status === 'active'}<Pause size={13} />{:else}<Play size={13} />{/if}
          </button>
          {#if kind === 'goal' && entry.status !== 'complete'}
            <button type="button" class="ghost icon small" disabled={saving || store.connection !== 'ready'} aria-label={strings.activity.finish} title={strings.activity.finish} onclick={() => void control('goal', 'complete')}><Check size={13} /></button>
          {/if}
          <button type="button" class="ghost icon small" disabled={saving || store.connection !== 'ready'} aria-label={strings.activity.remove} title={strings.activity.remove} onclick={() => void control(kind as 'goal' | 'loop', 'remove')}><X size={13} /></button>
        </div>
...
<style>
  .activity { width: 100%; max-width: var(--content); max-height: 45vh; overflow-y: auto; margin: 0 auto 8px; padding: 6px 10px; border: 1px solid var(--color-border); border-radius: var(--radius-lg); background: var(--color-surface); box-shadow: var(--shadow-e1); font-size: var(--text-sm); }
  ...
  @media (max-width: 720px) { .interval { display: none; } .activity-row { gap: 5px; } }
</style>
