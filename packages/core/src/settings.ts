import { settingsPatchError, type Settings } from '@boite/contracts';
...
export class SettingsStore {
  ...
  set(patch: Partial<Settings>): Settings {
    // One rule for the core and the in-memory client: `packages/contracts/src/settings-validation.ts`.
    const error = settingsPatchError(patch);
    if (error !== null) throw invalidParams(error.message, { field: error.field });
