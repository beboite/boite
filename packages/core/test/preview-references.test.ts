...
test('preview references reach drivers and continuation as untrusted context while visible prose stays compact', async () => {
  ...
  try {
    ...
    const turn = await client.call('turns.start', { threadId, prompt: 'Make the button clearer.', previewReferences: [reference], clientRequestId: 'preview-retry-1' });
    ...
    expect(part.displayText).toBe('Make the button clearer.');
    expect(part.previewReferences).toEqual([reference]);
    ...
    const retry = await client.call('turns.start', { threadId, prompt: 'Make the button clearer.', previewReferences: [reference], clientRequestId: 'preview-retry-1' });
    expect(retry.id).toBe(turn.id);
    await expect(client.call('turns.start', { threadId, prompt: 'Make the button clearer.', previewReferences: [{ ...reference, selector: '#different' }], clientRequestId: 'preview-retry-1' })).rejects.toThrow('different content');
